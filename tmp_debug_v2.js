...
const indexNum = '10327320';
const dob = '24-07-2007';
...
async function debug() {
    ...
    log('Warmup Status: ' + warmRes.status);
    log('Cookies: ' + getCookieHeader());
    ...
    log('Reset Status: ' + resetRes.status);
    log('Reset Response Length: ' + resetRes.body.length);
    ...
    log('Login Redirect: ' + loginRes.headers['location']);
    ...
    if (loginRes.status === 302 || loginRes.status === 301) {
        ...
        log('Transcript Status: ' + transRes.status);
        log('Transcript Body Preview: ' + transRes.body.substring(0, 300));
